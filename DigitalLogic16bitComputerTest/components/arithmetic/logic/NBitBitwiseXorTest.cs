...
{
    public class NBitBitwiseXorTest
    {
        ...
        public void TestNBitBitwiseXor(string numA, string numB, string expected)
        {
            var nBitA = NBitArray.BinaryStringToNBitArray(numA);
            var nBitB = NBitArray.BinaryStringToNBitArray(numB);
            var nBitXor = new NBitBitwiseXor(nBitA, nBitB);

            Assert.That(nBitXor.OutputNum.ToBinaryString(), Is.EqualTo(expected));
        }
    }
}
